...
original_author="baiyongzhen"
original_name="flask_example"
original_urlname="flask-example"
original_description="Awesome flask_example created by baiyongzhen" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_example "${name}"
...
then
    ...
    echo "then run 'flask_example' or 'python -m flask_example'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_example' or 'python -m flask_example' or 'poetry run flask_example'"
fi
